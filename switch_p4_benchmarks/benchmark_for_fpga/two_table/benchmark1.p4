...
register<bit<32>>(1) s0;
register<bit<32>>(1) s1;
struct headers {
    packets_t  pkts;
}
...
control ingress(inout headers hdr,
    ...
    action marple_tcp_nmo() {
        @atomic {
            bit<32> s0_tmp;
            bit<32> s1_tmp;
            s0.read(s0_tmp, 0);
            s1.read(s1_tmp, 0);
            if (hdr.pkts.pkt_1 < s0_tmp) {
                s1_tmp = s1_tmp + 1;
            } else {
                s0_tmp = hdr.pkts.pkt_1;
            }
            s0.write(0, s0_tmp);
            s1.write(0, s1_tmp);
        }
