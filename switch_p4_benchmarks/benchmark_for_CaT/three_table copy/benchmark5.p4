#include <core.p4>
#include <v1model.p4>
header packets_t {
bit<32> pkt_0;
bit<32> pkt_1;
bit<32> pkt_2;
bit<32> pkt_3;
bit<32> pkt_4;
bit<32> pkt_5;
bit<32> pkt_6;
bit<32> pkt_7;
bit<32> pkt_8;
bit<32> pkt_9;
bit<32> pkt_10;
bit<32> pkt_11;
bit<32> pkt_12;
bit<32> pkt_13;
bit<32> pkt_14;
bit<32> pkt_15;
bit<32> pkt_16;
}
struct headers {
    packets_t  pkts;
}

struct metadata {
}

parser MyParser(packet_in packet,
                out headers hdr,
                inout metadata meta,
                inout standard_metadata_t standard_metadata) {
    state start {
        packet.extract(hdr.pkts);
        transition accept;
    }

}

control MyVerifyChecksum(inout headers hdr, inout metadata meta) {
    apply {
    }
}

control ingress(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata) {
    
    action src_vtep_hit(bit<32> ifindex) {
        hdr.pkts.pkt_0 = ifindex;
    }
    table ipv6_src_vtep {
        actions = {
            
            src_vtep_hit;
        }
        key = {
            hdr.pkts.pkt_1                    : exact;
            hdr.pkts.pkt_2                        : exact;
            hdr.pkts.pkt_3 : exact;
        }
        size = 1024;
    }
    action multicast_bridge_star_g_hit_ipv6(bit<32> mc_index) {
        hdr.pkts.pkt_4 = mc_index;
        hdr.pkts.pkt_5 = 1;
    }
    table ipv6_multicast_bridge_star_g {
        actions = {
            multicast_bridge_star_g_hit_ipv6;
        }
        key = {
            hdr.pkts.pkt_6      : exact;
            hdr.pkts.pkt_7 : exact;
        }
        size = 1024;
    }
    action set_ecmp_nexthop_details(bit<32> ifindex, bit<32> bd, bit<32> nhop_index, bit<32> tunnel) {
        hdr.pkts.pkt_8 = ifindex;
        hdr.pkts.pkt_9 = nhop_index;
        hdr.pkts.pkt_10 = hdr.pkts.pkt_6 ^ bd;
        hdr.pkts.pkt_11 = hdr.pkts.pkt_11 ^ ifindex;
        hdr.pkts.pkt_12 = hdr.pkts.pkt_13 ^ tunnel;
    }
    action set_ecmp_nexthop_details_for_post_routed_flood(bit<32> bd, bit<32> uuc_mc_index, bit<32> nhop_index) {
        hdr.pkts.pkt_14 = uuc_mc_index;
        hdr.pkts.pkt_9 = nhop_index;
        hdr.pkts.pkt_8 = 0;
        hdr.pkts.pkt_10 = hdr.pkts.pkt_6 ^ bd;
        hdr.pkts.pkt_15 = 127;
    }
    table ecmp_group {
        actions = {
            set_ecmp_nexthop_details;
            set_ecmp_nexthop_details_for_post_routed_flood;
        }
        key = {
            hdr.pkts.pkt_9 : exact;
            hdr.pkts.pkt_16      : exact;
        }
        size = 1024;
    }

    apply {
        ipv6_src_vtep.apply();
        ipv6_multicast_bridge_star_g.apply();
        ecmp_group.apply();
    }
}

control egress(inout headers hdr,
                 inout metadata meta,
                 inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control MyComputeChecksum(inout headers  hdr, inout metadata meta) {
     apply {  }
}

control MyDeparser(packet_out packet, in headers hdr) {
    apply { }
}

V1Switch(
MyParser(),
MyVerifyChecksum(),
ingress(),
egress(),
MyComputeChecksum(),
MyDeparser()
) main;
